...
describes what users get. Pick a format with `-o`, or let ocwmit choose:
a table when writing to a terminal, JSONL when piped.

## Formats

```bash
ocwmit <command> -o table   # aligned columns for reading
ocwmit <command> -o jsonl   # one JSON object per line, for piping
ocwmit <command> -o json    # a single JSON array
ocwmit <command> -o csv     # spreadsheet friendly
ocwmit <command> -o tsv     # tab-separated
ocwmit <command> -o url     # just the URL column
ocwmit <command> -o raw     # the underlying bytes, unformatted
```
...
```bash
ocwmit <command> --fields id,title,url
```
...
```bash
ocwmit <command> --template '{{.URL}} {{.Title}}'
```
...
```bash
ocwmit <command>            # a table, because this is a terminal
ocwmit <command> | wc -l    # JSONL, because this is a pipe
```
